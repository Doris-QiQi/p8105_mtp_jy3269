...
### right plot -- Each Dog’s Weight Over Time
```{r}
weight_over_time <- weight_2 %>% 
  filter(unit == "lbs",
         dog != "std") %>% 
...
### right plot -- distribution of weights for each dog

```{r}
weight_distribution <- weight_2 %>% 
  filter(unit == "lbs",
         dog != "std") %>%
  ...
  labs(x = "Dog",
       y = "Weight  (lbs)",
       title = "Weight Distribution of Each Dog") +
  theme_bw() +
  theme(legend.position = "bottom")
...
weight_distribution + weight_over_time
