...
**Brief Introduction to the Raw Data:**
  ...
  * The data set is a record of 3 pet dogs' weight from `r weight$date[1]` to `r weight$date[-1]`. 
...
**Goals of the Report:**
  ...
  * Show and explain the major steps in turning the raw data set into a tidy data set available for analysis.
  * Explore the features and patterns in the 3 dogs' weight data and try to interpret them.
  * Visualize the distribution of dogs' weights and their variation over time and make conclusions from the plots.
...
The raw data is imported from an excel spreadsheet. Because of the merged cells, the column names are not correct in `raw_data`.
...
I solved this problem by extracting the first row as column names to rename the columns. Then, remove the previous first row to avoid duplication.
```{r}
...
head(weight)                  # show part of the transformed data

```


#### Issue 2: Unify the unit for weight measurement
...
weight <- weight %>% 
  ...
  janitor::clean_names() %>% 
  mutate(Raisin = raisin_lbs + raisin_oz/16,
...
#### Issue 3: Messy date format shown in R
