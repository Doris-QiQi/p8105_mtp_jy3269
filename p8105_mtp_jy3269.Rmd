...
### right plot -- Each Dog’s Weight (oz) Over Time
```{r}
weight_oz_over_time <- weight_2 %>% 
  filter(unit == "oz",
         dog != "std") %>% 
  ggplot(aes(x = date, y = weight,color = dog), group = dog) +
  geom_line() +
...
### left plot -- distribution of weights (oz )for each dog
```{r}
weight_distribution_oz<- weight_2 %>% 
  filter(unit == "oz",
         dog != "std") %>%
  ggplot(aes(x = dog, y = weight,fill = dog)) +
  geom_violin() +
