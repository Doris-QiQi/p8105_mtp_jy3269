...
The data set is a record of 3 pet dogs' weight from `r weight$date[1]` to `r weight$date[-1]`. The raw data set has `r nrows(raw_data)` rows and 6 columns. The first column is the date when the dogs are weighed. The next 3 columns show the weight of each dog, named Raisin, Simone and Gagne respectively.To ensure the measurement accuracy, a standard object is also weighed and recorded occasionally as a reference. The units used to measure in the raw data set is a combination of lbs and oz. The last column contains occasional notes for the dogs' status.
